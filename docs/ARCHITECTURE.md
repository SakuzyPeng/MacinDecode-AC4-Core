...
|---|---|---|---|
| presentation payload | `PresentationSubstreamMetadata::payload` 与 `parsed_substream()`；当前帧所有 presentation processing 字段及原始 bit view | Session 所选 presentation 的配置代次 | Session 复制完整 bounded payload；视图借用到下一次可变调用。`syntax_payload` 与 `compatibility_tail` 明确分开已知 `0x00`/`0x80`/`0xd8` 尾部，且不赋予尾部语义 |
