...
另以一份本地私有 768K 音乐参考素材的中置/q2 做受控观察：1024 点 Hann STFT、64 samples
...
### 5.57 `ac4_presentation_substream()` 的 selection、additional data、响度、DRC、group gain 与 associated audio
...
普通 presentation 没有 selection 前缀，公共 metadata 从 bit 0 开始；alternative 前缀结束位置
...
完整 `drc_frame()` 作为有界 bit view 保留；长度为零、越过 payload 或扩展溢出均拒绝。
TOC 的 `b_pres_ndot` 直接传给 P1 `4.2.14.5` 的 `drc_frame(b_iframe)`：DRC present 的
independent frame 按 `4.2.14.6`–`4.2.14.8` 解析 `drc_config()`，固定容量保存 1–8 个
decoder mode、内置/自定义输出电平、repeat/default/curve/gains 四类 profile、(E-)AC-3 profile
及 compression curve 的 boost/cut control points、time constants 和 adaptive thresholds 原值。
dependent frame 不猜测或重读配置；两种 frame 都保留完整 `drc_data()` bit view。independent
frame 还按 `4.2.14.9`/`4.3.13.5` 解析 repeat profile 的有效 gain/curve 形态、每个 gain mode
的 6 比特 `drc_gainset_size_value` 与 `variable_bits(2) << 6` 扩展、2 比特 `drc_version`，以及
...
gain 数组长度。逐帧 API 保留原始更新形态与码值，并返回随后 `b_associated` 的精确 offset；
`PresentationSubstreamGroupGainState` 另按 presentation 隔离有效数组。按 `6.3.3.1.23`，首次
接收 `b_keep = 0` 前使用全零的 0 dB 码；之后 keep 沿用前值，新值替换整个数组。presence 缺席
或单 group gate 不存在表示本帧无 group gain，状态归零；`b_pres_ndot` 独立帧先丢弃历史，保证
无需前帧即可求得有效值。未 reset 的 dependent frame 若改变 group 数则结构化失败且旧状态
不变；seek、换源、拓扑变化和不连续由调用方显式 reset。状态不做表 70 的 dB 换算或 gain 应用。

`6.3.3.1.25`–`6.3.3.1.26` 再解析 `b_associated`，三个 scale presence 彼此独立；存在时保留
...
默认值。`OamdAlternativeDataSetState` 因而要求调用方按 `(physical audio substream, domain,
data_set_index)` 隔离实例，其中 domain 区分 non-A-JOC、A-JOC core/downmix 与 full/upmix：非 keep
更新拥有化保存当前规范已定义的 gain、标准精度位置和 `ext_prec_alt_pos()` 原值，dependent keep
返回同一快照；首次 keep、reset 后 keep 与 I-frame keep 均失败关闭。dependent frame 未 reset
改变对象布局或把同一实例用于另一 domain/index 也结构化失败，且任何失败都不提交候选。domain
由共享解析器随 dataset 携带，因此同一 A-JOC 物理 substream 内两份同 index、同布局列表也不能
静默互相覆盖。按单 domain/index 建模避免为无固定小上限的 dataset 列表引入任意容量限制，也
不会代替 presentation target 做选择。additional-data 中规范未定义的 `skip_data` 继续由当前帧
零拷贝 view 完整保留；状态不猜测未来扩展是否属于 keep 的属性集合。
...
dataset domain/index 串用、A-JOC core/full 同布局隔离、dependent 对象布局变化和失败事务。真实
alternative/direct-object 向量仍待后续。Channel-based 没有对象 metadata 上下文，按既定范围
继续失败关闭。
...
**本节的政策依据是 [ADR-0016](decisions/0016-accept-non-normative-stream-behaviour.md)。**
决策与边界（为什么可以有这个例外、它为什么不侵蚀 fail-closed、严格路径为什么不能动）在那份
ADR；条目在本节表中。两处不重复存放同一份数据。

与第 6、7 节都不同：**本节的每一条，规范在那个位置根本没有可读的东西。** 第 6 节是规范说了
但自相矛盾或不完整，需要先形成结论；第 7 节是规范说了、实现已选定一种读法、等外部裁决；本节
是码流里出现了任何条款都不产生的数据，而真实工具链稳定地写出它。

**外部参考解码器裁决不了本节。** 它对这些字节的处理同样是产品行为而非规范，与它一致既不能
证明读法正确，也不能证明该数据合法。第 7 节"接上 oracle 后按图索骥"的方法在这里不适用；本节
的裁决手段只有两条：更多真实样本，以及向 ETSI 确认该行为是否另有条款依据。

**最大的风险是把一处真实的漏读粉饰成「编码器写了个怪字节」。** 因此登记本表有三个前置条件，
缺一不可：

1. **规范语法恰好耗尽。** 必须先证明严格路径在该位置正常结束——剩余比特数、对齐状态和最后
   一个规范字段都要能指出来。证明不了的不是本节的条目，而是一个解析缺陷，或第 6 节的未决
   问题。
2. **有真实样本与命中统计。** 记录样本 SHA-256、命中帧数与总帧数。单帧孤例只能记录为观察，
   不足以登记为稳定行为并据此放宽解析。
3. **检索过约束该字段的条款，而不只是检索过该符号。** CLAUDE.md 记过这条教训：`5_X`/`7_X`
   全文检索找不到定义，而 P1 表 67 用 `if (5 ≤ ch_mode ≤ 10)` 重述了同一约束，原理上检索
   不到。本表的条目随时可能因为找到等价条款而降级为普通判读——那是好事，但前提是当初记下了
   检索范围。

**严格路径永不改变。** 兼容只能是独立入口，由调用方显式选择使用；这样「我们对规范的理解」与
「我们对现实的让步」在代码里始终分得开，任何一天想退回纯规范行为都只是不调用那个入口。表中
「严格路径」一列若出现「已改变」，该条目必须升级为第 6 节的未决问题重新论证。

**样本不入库，哈希入库。** 非自产素材的音频与原始 payload 都不进版本控制。

| 现象 | 上下文条款 | 来源与命中 | 兼容集合与门槛 | 严格路径 | 漏读排除依据 |
|---|---|---|---|---|---|
| independent object/A-JOC presentation payload 在末尾 `byte_align()` 之后多出一个完整字节 | P2 `6.2.2.3`；该节对齐之后不再定义任何字段 | 自产 DME/DEE 链的 12 条 A-JOC 基线向量，首个 independent presentation payload：7 条 `0x00`、5 条 `0x80` | `{0x00, 0x80}`。要求 independent、object/A-JOC、本帧携带 DRC configuration，且严格路径恰好剩余一个完整字节 | 未改变，仍按规范拒绝任何完整尾随字节 | 严格解析恰好剩余 8 比特；`0x81`、DRC absent 与 channel 上下文均有构造拒绝门禁；三层 `decode_check.py` 对 12 条向量仍逐位命中原基线 |
| 同一位置出现第三个取值 `0xd8` | P2 `6.2.2.3`（同上） | 2026-09-08 本地音乐回放回归，**非自产**样本 `d59b96ae125fdb16077608d611d7e64730880478a7e918c350760056be906d48`；6009 个 AU 中 252 个 independent object/A-JOC 帧全部命中，其余 5757 帧无尾部 | 扩为 `{0x00, 0x80, 0xd8}`，门槛不变 | 未改变 | 规范语法在完整 DRC configuration 与 `loud_corr()` 之后、bit offset 128 处结束，`alignment_bits == 0`，尾字节位于 16 字节前缀之后；构造测试遍历其余 253 个字节验证拒绝与状态回滚 |

实现位置：兼容入口 `Ac4PresentationSubstream::parse_with_drc_state_compat` 与严格入口
`parse_with_drc_state` 并存于 `macindecode-ac4-bitstream`；`macindecode-ac4-scene::session` 与
`macindecode-ac4-metadata::presentation` 显式选择兼容入口。逐条数据、命中统计与回归证据见
[测试向量策略](TEST_VECTOR_STRATEGY.md) 9.2m。

**新增条目要同步登记此表。** 本节的发现渠道不限于自产向量——`0xd8` 就来自播放一份非自产文件，
而自产编码链至今不产生它。这类条目散落在各模块的常量注释里时，「现在一共几条、门槛是否一致、
有没有哪一条其实是我们漏读」只能靠回忆，而回忆会漏。
