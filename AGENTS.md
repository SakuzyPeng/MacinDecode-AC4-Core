...
This Rust 2024 workspace has an MSRV of 1.98. Workspace crates live under `crates/`: `crates/macindecode-ac4-bitstream/` owns bounded syntax and metadata, `crates/macindecode-ac4-decode/` owns ASF/A-SPX/A-JOC reconstruction and the Full engine, `crates/macindecode-ac4-scene/` owns the public scene API, `crates/macindecode-ac4-mp4/` owns containers and timing, `crates/macindecode-ac4-inspect/` owns the public inspection report, and `crates/macindecode-ac4-cli/` owns trace, validation, and exports. `crates/macindecode-ac4-perf/` is an unpublished internal performance harness. Tests sit beside implementations. Use `docs/` for design records, `scripts/` for utilities, and `vectors/<case_id>/` for case metadata. `spec/MANIFEST.json` pins specification sources.
