...
MP4/raw AC-4 trace，以及 PCM WAVE、ADM BWF、DAMF 和 Apple CAF 导出。
...
默认构建可执行容器、TOC/OAMD/metadata trace；需要音频解码的导出命令返回稳定的
...
成功时 stdout 使用带 `schema`/`version` 的 JSON v1 envelope；失败时 stdout 为空，
诊断写入 stderr。完整命令和输出契约见
