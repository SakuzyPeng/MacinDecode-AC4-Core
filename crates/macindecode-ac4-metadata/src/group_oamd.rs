...
use crate::{
    MetadataError as DecodeError, MetadataErrorContext as DecodeErrorContext,
    MetadataErrorKind as DecodeErrorKind,
};

pub type OamdObserver<'a> = dyn FnMut(u32, &OamdSubstreamPayload, OamdState, &ObjectDescriptors) -> Result<(), DecodeError>
    + 'a;
...
pub struct PreparedGroupOamdState {
    pub group_index: u32,
    pub effective_common: Option<OamdCommonData>,
    pub common_updated_in_source_access_unit: bool,
    pub effective_timing: Option<OamdTimingData>,
    pub timing_updated_in_source_access_unit: bool,
    pub content_classifier: Option<u8>,
}
...
struct PhysicalOamdUpdate {
    ...
    timing_updated: bool,
    descriptors: Option<ObjectDescriptors>,
    ndot: bool,
    alternative: bool,
}
...
impl PreparedGroupOamd {
    ...
    #[must_use]
    pub fn groups(&self) -> &[PreparedGroupOamdState] {
        self.groups.get(..self.groups_written).unwrap_or(&[])
    }

    #[must_use]
    pub const fn group_num_obj_info_blocks(&self) -> Option<u8> {
        ...
        match self.group_num_obj_info_blocks {
            ...
            Some(current) => {
                return Err(DecodeError::new(
                    DecodeErrorKind::OamdTimingConflict {
                        expected: current,
                        actual: next,
                    },
...
impl GroupOamdDecoder {
    ...
    /// 巡检按每个 group 保留 timing；observer 在候选上运行，失败时须丢弃本批事件。
        ...
        for (group_position, group) in topology.groups().iter().enumerate() {
            ...
            let external = match group.oamd_substream {
                Some(reference) => {
                    let Some(substream_index) = reference.substream_index else {
                        return Err(DecodeError::new(
                            DecodeErrorKind::Unsupported("OAMD substream index absent"),
                    ...
                    match prepared.physical_update(substream_index) {
                        Some(update) => {
                            let descriptors =
                                ObjectDescriptors::from_group(group).map_err(|e| {
                                    DecodeError::new(
                                        DecodeErrorKind::Oamd(e),
                                        scoped_context(scope, group_index, Some(substream_index)),
                                    )
                                })?;
                            if update.descriptors != Some(descriptors)
                                || update.ndot != reference.ndot
                                || update.alternative != group_is_alternative(topology, group_index)
                            {
                                return Err(DecodeError::new(
                                    DecodeErrorKind::ContextConflict,
                                    scoped_context(scope, group_index, Some(substream_index)),
                                ));
                            }
                            Some(update)
                        ...
                        None => {
                            let update = self.prepare_physical(
                                PhysicalOamdRequest {
                                    raw_frame,
                                    topology,
                                    group,
            ...
            if merge_timing {
                prepared.merge_block_count(
                    effective_timing.map(|t| t.num_obj_info_blocks),
                    scope,
                    group_index,
                )?;
            }
    ...
    fn prepare_physical(
        ...
        let payload = topology
            ...
            .map_err(|error| {
                DecodeError::new(
                    DecodeErrorKind::Topology(error),
                    scoped_context(scope, group_index, Some(substream_index))
        ...
        state.apply(&parsed).map_err(|error| {
            DecodeError::new(
                DecodeErrorKind::OamdState(error),
                scoped_context(scope, group_index, Some(substream_index))
                    .with_syntax_path(OAMD_SYNTAX),
            )
        })?;

        observer(substream_index, &parsed, initial, &descriptors)?;
        Ok(PhysicalOamdUpdate {
            descriptors: Some(descriptors),
            ndot,
            alternative: group_is_alternative(topology, group_index),
            substream_index,
            state,
    ...
    pub fn invalidate_group(&mut self, topology: &Ac4Topology, group_index: usize) {
        if let Some(common) = self.group_common.get_mut(group_index) {
            *common = None;
        }
        if let Some(index) = topology
            ...
            && let Some(state) = self.physical.get_mut(index)
        {
            state.reset();
        }
    }
    pub fn commit(&mut self, prepared: &PreparedGroupOamd) {
...
fn common_conflict(
    ...
    DecodeError::new(
        DecodeErrorKind::OamdCommonConflict,
...
mod tests {
    ...
    fn conflicting_common_values_fail_with_group_and_substream_context() {
        ...
        assert_eq!(error.kind(), DecodeErrorKind::OamdCommonConflict);
