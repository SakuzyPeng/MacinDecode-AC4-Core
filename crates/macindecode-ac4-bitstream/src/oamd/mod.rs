...
//! [`OamdAlternativeDataSetState`] 可由两条路径共享，用于按物理 audio substream、
//! non-A-JOC/A-JOC core/full domain 与 dataset loop index 延续 dependent frame 的 `b_keep`
//! 有效属性；它拥有化保存规范已定义的 gain 与位置原值，opaque additional data 仍由当前帧的
//! 只读 view 保留。
//!
//! # 原始量化值
//!
